...
@TeleOp(name = "JewelTest2", group = "Linear Opmode")
//@Disabled
public class JewelTest2 extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        ((Activity)hardwareMap.appContext).runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                button.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) { try { processJewels(); } catch (InterruptedException e) {/*meh*/} }
                });
        ...
        closeIDs(BUTTON_ID, vh.getPreview().getId(), IMG2_ID);

    }

    private View findViewID(int id) { return ((Activity)hardwareMap.appContext).findViewById(id); }

    private void closeIDs (int... ids) {
        for (final int id:ids) {
            ((Activity)hardwareMap.appContext).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    try { vh.getPreviewContainer().removeView(findViewID(id)); }
