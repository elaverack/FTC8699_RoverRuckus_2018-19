...
public class AlignmentCircle {
    ...
    Mat getMask (Mat mat, Enums.Color color) {
        Mat clone = mat.clone();
        isolate(clone);
        Mat ret = clone.clone();
        if (color == Enums.Color.RED) {
            Core.inRange(clone, VisualsHandler.RED_L, VisualsHandler.RED_H, ret);
        } else if (color == Enums.Color.BLUE) {
            Core.inRange(clone, VisualsHandler.BLUE_L, VisualsHandler.BLUE_H, ret);
        }
        return ret;
    }

    public Enums.Color getJewelColor(Mat mat) {
        ...
        if (redm.get_m00() > bluem.get_m00()) return Enums.Color.RED;
        if (bluem.get_m00() > redm.get_m00()) return Enums.Color.BLUE;
        return Enums.Color.ERROR;
    }
