...
public class Vuforia2017Manager {
    ...
    public Vuforia2017Manager(boolean doSave) {
        VuforiaLocalizer.Parameters params = new VuforiaLocalizer.Parameters(com.qualcomm.ftcrobotcontroller.R.id.cameraMonitorViewId);
        params.cameraDirection = VuforiaLocalizer.CameraDirection.BACK;
        params.vuforiaLicenseKey = "";
        params.cameraMonitorFeedback = VuforiaLocalizer.Parameters.CameraMonitorFeedback.AXES;
    ...
    private int[] processImageOpenCV(Bitmap bmp, Scalar blueLow, Scalar blueHigh) {
        ...
        savePhoto(mask, "mask.png");

        log.log("" + mask.cols() + ", " + mask.rows() + ", " + mask.width() + ", " + mask.height());

        if (mmnts.get_m00() / mask.total() > 0.5) {
            config =  BLUE_BLUE;
            int[] Return = {config, width, height};
            log.log("" + (mmnts.get_m00() / mask.total()));
            return Return;
        ...
        if ((mmnts.get_m10()) / (mmnts.get_m00()) <  image.cols() / 2) {
            config =  BLUE_RED;
            log.log("" + mmnts.get_m10() + ", " + mmnts.get_m00() + ", " + image.cols());
            int[] Return = {config, width, height};
            return Return;
        } else {
            config =  RED_BLUE;
    ...
    private Image getImageFromFrame(VuforiaLocalizer.CloseableFrame frame, int pixelFormat) {
        ...
        for (int i = 0; i < numImgs; i++) {
            if (frame.getImage(i).getFormat() == pixelFormat) {
                ...
                log.log("Not the image I'm looking for. Format: " + frame.getImage(i).getFormat());
            }
        }
    ...
    private Bitmap getBeacon(Image img, VuforiaTrackableDefaultListener beacon, CameraCalibration camCal) {
        ...
        if (pose == null) {
            log.log("Ahh! Pose is null!");
            return null;
        }

        if (img == null) {
            log.log("Ahh! Image is null!");
            return null;
        } else if (img.getPixels() == null) {
            log.log("Ahh! Image pixels are null!");
            return null;
        } else {

            log.log("Saving start.png...");
            ...
            savePhoto(Return, "croppedBeacon.png");
    ...
    public boolean canSee (int trackableIndex) {
        return beacons.get(trackableIndex) != null;
    }
